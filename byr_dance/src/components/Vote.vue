...
export default {
  ...
  methods: {
    getSourceData(){
      ...
      this.musicList=[]
      this.bk=[]
      axios.get( 'http://ivantseng.cn:8080/music/getTop').then((res) => {
    ...
    submit(){//提交数据
      if(this.selected==0&&this.inputValue==''){
        ...
        this.loading=true
        axios.post( 'http://ivantseng.cn:8080/music/addCount',{name:this.inputValue||this.selectedName}).then((res) => {
